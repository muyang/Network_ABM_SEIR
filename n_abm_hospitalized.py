...
df_res.columns=['tick','S','E','I','Is','R','D','ts']   
	...
	print((tick,sNum,eNum,iNum,isNum,rNum,dNum,ts))
########################
#spreading
########################
mean_connections = param.Number(default=6, doc="""	Mean number of connections to make to other nodes.""")
pSick = param.Number(default=0.01, doc="""Probability of a node to be initialized in sick state.""", bounds=(0, 1))
pVaccinated = param.Number(default=0.1, bounds=(0, 1), doc="""Probability of a node to be initialized in vaccinated state.""")
pInfect = param.Number(default=0.3, doc="""Probability of infection on each time step.""", bounds=(0, 1))
pRecover = param.Number(default=0.05, doc="""Probability of recovering if infected on each timestep.""", bounds=(0, 1))
pDeath = param.Number(default=0.1, doc="""Probability of death if infected on each timestep.""", bounds=(0, 1))	
'''
# a.py
from pymc import *
...
def spreading_init(place_in_risk):
	"""Initialise the network with vaccinated, susceptible and infected states."""
	#place_in_risk=get_pir()
	#
	#g = nx.watts_strogatz_graph(N*10, 6, 0.2, seed=None)
	vaccinated, infected = 0, []   #infected => people_infected
	for i in g.nodes.keys():
		g.nodes[i]['transmissions'] = 0
		if(rnd.random() <= pVaccinated): 
			g.nodes[i]['state'] = 'Q'			   #quarantine
			vaccinated += 1
		elif(rnd.random() <= pSick):
			g.nodes[i]['state'] = 'I'
			infected.append(i)
		else:
			g.nodes[i]['state'] = 'S'
			...
			for m in g.neighbors(i):
				if g.nodes[m]['state'] == 'S':
					if rnd.random() <= pInfect:
						g.nodes[m]['state'] = 'I'
						infected.append(m)	   #infected => people_infected
						#g.nodes[i]['transmissions'] += 1	#for stats

			# recover with probability pRecover
			if rnd.random() <= pRecover:
				g.nodes[i]['state'] = 'R'
			elif rnd.random() <= pDeath:
				edges = [edge for edge in g.edges() if i in edge] 
				g.nodes[i]['state'] = 'D'
				g.remove_edges_from(edges)
...
#########################################################################################
fig = plt.figure()
folder_tmp_files = r"C:\Users\mu.yang\Documents\Mu\Penny\networkx\datasets\animation"
def animate(mnod):
