...
to remove-node
  ask turtles [
    ;if ticks > (t0 + duration) [ask my-links [die] set color grey set removed? true]
    ifelse duration > 0 [set duration duration - 1 ]
                        [ask my-links [die] set color grey set removed? true]
  ]
end
...
to make-node [old-node]
  ...
  [
    set color red
    set t0 0
    set duration int(random 30 + 60)
    if old-node != nobody and duration > 0
...
63
hidden-grey
ask turtles with [color = grey] [set hidden? true]
NIL
1
T
OBSERVER
...
"" ""
PENS
"default" 1.0 1 -16777216 true "" "histogram [duration] of turtles with [color = red]"
