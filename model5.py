...
def step(ticks,interval,param,pdf,gdf,pInfect):
	...
	for tick in range(0,ticks,interval):
		...
		if h < 8 and h > 22:
			...
			pdf['pos'][(pdf.state=='Is')&(pdf.Hospitalization==False)]=gdf.loc[gdf.Level2==402].gid.sample(l,replace=True)
		
		#people_infected=pdf.loc[(pdf.state.isin(['E','I','Is']))&(pdf.Hospitalization==False)]
		x1=pdf.loc[(pdf.state.isin(['E','I','Is']))&(pdf.Hospitalization==False)].shape[0]
		x2=0

		place_in_risk=pdf.loc[(pdf.state.isin(['E','I','Is']))&(pdf.Hospitalization==False)].pos.unique()
